...
line_bot_api = LineBotApi(
    'bHi/8szU2mkZAaIMLGDKqTE8CnG4TjilHVVJsqDse2XD39ZUGdxiHRedvOGSC5Q7zJfFYZoOAIoMxeKAR5mQqbz0DomlYKjU7gMEK/zQ0QJFFVJLpDhwB8DRrJ8SAoqK+sEAMuD2PL0h0wdsZxncRwdB04t89/1O/w1cDnyilFU=')
        ...
        if '分帳設定' in receivedmsg:
            ...
            add_data = usermessage(
                ...
                birth_date=datetime.fromtimestamp(
                    int(bodyjson['events'][0]['timestamp'])/1000)
            )
...
def get_groupPeople(history_list, mode):
    ...
    SetMsgNumber = usermessage.query.filter(
        usermessage.group_id == selfGroupId).filter(usermessage.status == 'set').count()
    data_UserData = usermessage.query.filter(
        usermessage.group_id == selfGroupId).filter(usermessage.status == 'set')
    history_dic = {}
    history_list = []
...
def get_accountList():
    data_UserData = usermessage.query.order_by(
        usermessage.birth_date.desc()).limit(1).all()
    history_dic = {}
    history_list = []
    for _data in data_UserData:
        ...
        history_dic = {}
    selfId = history_list[0]['user_id']
    data_UserData = usermessage.query.order_by(usermessage.birth_date).filter(
        usermessage.user_id == selfId).filter(usermessage.status == 'save').filter(usermessage.type == 'user')
    history_dic = {}
    history_list = []
    count = 0
    for _data in data_UserData:
    ...
    for i in range(count):
        try:
            ...
            money = 0
        final_list.append(
            str(history_list[i]['Mesaage'])+' '+str(history_list[i]['Account']))
        add += money
...
def get_settleList():
    ...
    for _data in data_UserData:
        ...
        history_dic = {}
    selfGroupId = history_list[0]['group_id']
    dataSettle_UserData = usermessage.query.filter(usermessage.group_id == selfGroupId).filter(
        usermessage.status == 'save').filter(usermessage.type == 'group')
    historySettle_dic = {}
    historySettle_list = []
    ...
    for i in range(len(historySettle_list)):
        count += 1
        final_list.append(str(historySettle_list[i]['Mesaage'])+' '+str(
            historySettle_list[i]['Account'])+' '+str(historySettle_list[i]['GroupPeople']))
    perfect_list = ''
    for j in range(count):
        perfect_list = perfect_list+str(j+1)+'.'+str(final_list[j])+'\n'
    return perfect_list
...
def settle():
    ...
    for i in range(dataNumber):
        ...
        for j in range(len(duplicate)):
            place = get_groupPeople(
                history_list, 2).index(duplicate[count])
    ...
    for i in range(len(person_list)-1):
        ...
        if min == 0 or max == 0:
            ...
            result = result + \
                str(min_tuple[0])+'付給'+str(max_tuple[0]) + \
                str(abs(round(min, 2)))+'\n'
...
def handle_message(event):
    ...
    if history_list[0]['type'] == 'user':
        if (history_list[0]['Status'] == 'save') and ('記帳' in input_text):
            ...
            selfId = history_list[0]['user_id']
            data_UserData = usermessage.query.filter(usermessage.user_id == selfId).filter(
            ...
            selfId = history_list[0]['user_id']
            data_UserData = usermessage.query.filter(usermessage.user_id == selfId).filter(
                usermessage.status == 'save').filter(usermessage.type == 'user')
            history_dic = {}
            history_list = []
            count = 0
            for _data in data_UserData:
            ...
            if targetNum > count:
                ...
                data_UserData = usermessage.query.order_by(usermessage.birth_date).filter(usermessage.user_id == selfId).filter(
                    usermessage.status == 'save').filter(usermessage.type == 'user')[targetNum-1:targetNum]
                history_dic = {}
                history_list = []
                count = 0
                for _data in data_UserData:
        ...
        if (history_list[0]['Status'] == 'set') and ('分帳設定' in input_text):
            ...
            selfGroupId = history_list[0]['group_id']
            data_UserData = usermessage.query.filter(
                usermessage.group_id == selfGroupId).filter(usermessage.status == 'save').delete()
            output_text = '刪除成功'
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text=str(output_text)))
            ...
            selfGroupId = history_list[0]['group_id']
            data_UserData = usermessage.query.filter(
                usermessage.group_id == selfGroupId).filter(usermessage.status == 'set').delete()
            output_text = '設定刪除成功'
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text=str(output_text)))
            ...
            selfGroupId = history_list[0]['group_id']
            data_UserData = usermessage.query.filter(
                usermessage.group_id == selfGroupId).filter(usermessage.status == 'save')
            history_dic = {}
            history_list = []
            count = 0
            for _data in data_UserData:
            ...
            if targetNum > count:
                ...
                data_UserData = usermessage.query.order_by(usermessage.birth_date).filter(
                    usermessage.group_id == selfGroupId).filter(usermessage.status == 'save')[targetNum-1:targetNum]
                history_dic = {}
                history_list = []
                count = 0
                for _data in data_UserData:
                ...
                sys.stdout.flush()
                data_UserData = usermessage.query.filter(
                    usermessage.id == targetID).delete()
                time.sleep(2)
                output_text = '刪除成功'+'\n\n'+'分帳清單：'+'\n'+get_settleList()
                line_bot_api.reply_message(
                    event.reply_token,
